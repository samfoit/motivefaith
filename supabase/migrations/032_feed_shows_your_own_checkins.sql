-- ============================================================
-- 032_feed_shows_your_own_checkins.sql — a shared thread's preview
-- should say what the thread says
-- ============================================================
--
-- A friend's row in the feed is a preview of the conversation behind it. Open
-- the row and `get_friend_journey` shows every completion on the habits the
-- two of you share, from either of you. Close it again and the row said "No
-- shared activity yet", because `get_feed_friends` was counting only the
-- friend's completions:
--
--     JOIN completions c ON c.habit_id = s.habit_id
--       AND c.user_id != p_user_id        -- <- this
--
-- So checking in on your own habit — the ordinary case, the thing the partner
-- was invited to watch — left the row looking empty while the thread it opens
-- showed the check-in. Reproduced before changing anything: one accepted
-- partnership, one completion by the owner, and the feed returned
-- latest_completion = NULL while the journey returned one completion item.
--
-- The filter came from 014_feed_exclude_own_completions, whose stated aim was
-- to stop "the current user's own completions" appearing "under a friend's
-- row". But the page was already written to handle exactly that, and had been
-- since the first deployment: `buildFriendRows` renders
--
--     const isMe = comp.user_id === userId;
--     const actor = isMe ? "You" : profile.display_name.split(" ")[0];
--
-- which no row could ever reach once 014 landed. The display layer had solved
-- the display problem; 014 solved it a second time in the data layer and took
-- the legitimate case with it. The group rows never had this — they show
-- "You completed X" — so the two halves of the same screen disagreed.
--
-- Removing the filter is the whole change; the body is otherwise reproduced
-- from 029 unaltered.
--
-- Unread state is not affected. It comes from unread `encouragements`
-- addressed to the viewer, never from `latest_completion`, so a row cannot
-- start claiming to be unread because of something the viewer did themselves.
