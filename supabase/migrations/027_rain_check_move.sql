-- ============================================================
-- 027_rain_check_move.sql — a rain check can be moved, not just skipped
-- ============================================================
--
-- Builds on 025, which introduced 'rain_check' as a completion_type and the
-- rule that a rain-check row holds the streak simply by existing on a date.
--
-- A rain check now carries an optional `rain_check_moved_to` day: "not today,
-- but I will do it on Thursday". That turns the skip into a promise, and a
-- promise can be broken. So the invariant 025 relied on —
--
--     a row exists on this date  ==  this day is covered
--
-- gains one clause. A rain check with a moved-to day covers its own date only
-- while the promise is pending or has been kept; once the moved-to day has
-- passed with no real completion on it, the row stops covering and the streak
-- breaks. A rain check with no moved-to day is unchanged: a plain skip that
-- covers its day forever.
--
-- The new clause lives in exactly one place, is_habit_day_covered(), so the
-- three readers of the invariant cannot drift apart.

-- ── Column ──

ALTER TABLE public.completions
  ADD COLUMN IF NOT EXISTS rain_check_moved_to DATE;

ALTER TABLE public.completions
  DROP CONSTRAINT IF EXISTS chk_rain_check_moved_to;

-- Forward-only, and never further out than 14 days: both update_streak() and
-- computeEffectiveStreak() (TS) walk back at most 14 days looking for the
-- previous scheduled day, so a promise beyond that window could never be seen.
-- completed_date is already populated here — trg_set_completed_date is a BEFORE
-- trigger, and CHECK constraints are evaluated after it.
ALTER TABLE public.completions
  ADD CONSTRAINT chk_rain_check_moved_to CHECK (
    rain_check_moved_to IS NULL
    OR (
      completion_type = 'rain_check'
      AND rain_check_moved_to > completed_date
      AND rain_check_moved_to <= completed_date + 14
    )
  );

-- Serves the "was the promise kept?" lookup below, which probes a single
-- (habit_id, completed_date) pair. idx_completions_habit_date_key from 001
-- already covers it; nothing new is needed.

-- ── The covering predicate ──
--
-- Mirrored by coversDay() inside computeEffectiveStreak() in
-- src/lib/utils/streak.ts. Keep the two in step.

CREATE OR REPLACE FUNCTION public.is_habit_day_covered(
  p_habit_id UUID,
  p_date     DATE,
  p_today    DATE
)
...
AS $$
  SELECT EXISTS (
    ...
    WHERE c.habit_id = p_habit_id
      ...
      AND (
        -- A real completion always covers its day.
        c.completion_type <> 'rain_check'
        -- A plain skip covers its day forever.
        OR c.rain_check_moved_to IS NULL
        -- A promise not yet due. Using >= rather than > also makes the day of
        -- the makeup itself count, which is what lets update_streak() see the
        -- original day as covered while inserting the makeup completion.
        OR c.rain_check_moved_to >= p_today
        -- A promise that was kept.
        OR EXISTS (
          SELECT 1 FROM completions m
          WHERE m.habit_id = c.habit_id
            AND m.completed_date = c.rain_check_moved_to
            AND m.completion_type <> 'rain_check'
        )
      )
  );
$$;

REVOKE ALL ON FUNCTION is_habit_day_covered(UUID, DATE, DATE) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION is_habit_day_covered(UUID, DATE, DATE) TO authenticated;

-- ── insert_completion: new rain_check_moved_to parameter ──
-- Same trap as 025: a defaulted extra parameter creates an OVERLOAD rather
-- than replacing the function, which would make every existing 5-argument call
-- ambiguous. Drop the old signature first; the grants go with it.

DROP FUNCTION IF EXISTS insert_completion(UUID, completion_type, TEXT, TEXT, rain_check_reason);
...
CREATE OR REPLACE FUNCTION insert_completion(
  ...
  p_rain_check_moved_to DATE DEFAULT NULL
)
RETURNS completions
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Ignore a reason or a moved-to day sent alongside a non-rain-check type
  -- rather than letting a malformed client trip the CHECK constraints.
  v_reason   := CASE WHEN p_completion_type = 'rain_check'
                     THEN p_rain_check_reason
                     ELSE NULL END;
  v_moved_to := CASE WHEN p_completion_type = 'rain_check'
                     THEN p_rain_check_moved_to
                     ELSE NULL END;

  INSERT INTO completions (
    habit_id, user_id, completion_type, evidence_url, notes,
    rain_check_reason, rain_check_moved_to
  )
  VALUES (
    p_habit_id, auth.uid(), p_completion_type, p_evidence_url, p_notes,
    v_reason, v_moved_to
  )
...
REVOKE ALL ON FUNCTION insert_completion(UUID, completion_type, TEXT, TEXT, rain_check_reason, DATE) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION insert_completion(UUID, completion_type, TEXT, TEXT, rain_check_reason, DATE) TO authenticated;

-- ── insert_completions_batch: carry the moved-to day through ──
...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN (item->>'completion_type')::completion_type = 'rain_check'
         THEN NULLIF(item->>'rain_check_moved_to', '')::date
         ELSE NULL END AS rain_check_moved_to
...
-- ── update_streak: a lapsed promise no longer bridges the gap ──
-- Body reproduced from 025. The only change is the prev_completion lookup,
-- which now asks is_habit_day_covered() rather than merely "does a row exist".
-- A 30-day floor keeps the per-row predicate cheap; expected_prev never walks
-- back more than 14 days, so nothing beyond that could matter.
...
BEGIN
  ...
  SELECT MAX(c.completed_date) INTO prev_completion
    ...
    WHERE c.habit_id = NEW.habit_id
      ...
      AND c.completed_date >= NEW.completed_date - 30
      AND public.is_habit_day_covered(
            c.habit_id, c.completed_date, NEW.completed_date);
...
-- ── reset_stale_streaks: a lapsed promise leaves the day uncovered ──
-- Body reproduced from 012. Both CTEs swap their bare NOT EXISTS row check for
-- NOT is_habit_day_covered(...). This is the function that actually breaks a
-- streak the morning after a promise was not kept — update_streak() only ever
-- runs on an insert.
...
BEGIN
  ...
  -- ── Reset weekly habits with no covered day in current or previous week ──
...
-- ── get_incomplete_habits_today: a promised makeup is due today ──
-- Body reproduced from 004_functions.sql. Without this arm a moved occurrence
-- gets no reminder and the streak breaks silently the next morning.
...
-- ── notify_completion: say where the habit moved to ──
-- Body reproduced from 025; only the rain-check title differs. A move is
-- better news than a skip, and saying the day is what makes it a promise
-- a partner can hold you to.
...
-- ── Journey + group timeline RPCs: carry rain_check_moved_to ──
-- Bodies reproduced from 025 (themselves from 016 and 017) with the moved-to
-- day threaded through, so a partner sees "moved to Thursday" rather than a
-- bare rain check. Both return JSONB payloads, so the signatures are unchanged.
-- (from 016)
